...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // Si HelloAsso échoue, on n'enregistre rien : l'adhérent doit repayer.
    ...
    // ── Insertion dans inscriptions_publiques (statut: paiement_en_attente) ──
    // ↳ La fiche adherents et la vente factures sont créées par status.js
    //   uniquement après confirmation du paiement HelloAsso.
    ...
    const row = {
      ...
      adherent_id: null, // sera rempli par status.js après paiement
    ...
    // ── Email d'alerte avec le lien HelloAsso ────────────────────────────────
    ...
    // ── Réponse au front ─────────────────────────────────────────────────────
    // Le front doit rediriger vers helloAssoUrl immédiatement.
    return json({
      data: {
        registrationId,
        total: totals.total,
        helloAssoUrl,        // ← le front redirige vers cette URL
        helloAssoCheckoutIntentId,
      },
      error: null,
    });
